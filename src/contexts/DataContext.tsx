...
export const DataProvider = ({ children }: { children: ReactNode }) => {
    ...
    const updateOrganizationStatus = async (id: string, status: 'approved' | 'rejected') => {
        ...
        if (isSupabaseConfigured() && user && !isDemoUser(user.id)) {
            const { data, error } = await supabase.rpc('update_org_status', {
                org_id: id,
                new_status: status,
            });

            if (error) {
                console.error("RPC update_org_status failed:", error);
                fetchData();
                throw new Error(error.message || 'Failed to update organization status');
            }

            if (data && !data.success) {
                console.error("Org status update rejected:", data.error);
                fetchData();
                throw new Error(data.error || 'Update was rejected');
            }

            console.log("Org status updated successfully:", data?.name, "→", status);
        }
    };
